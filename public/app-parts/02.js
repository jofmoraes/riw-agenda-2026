...
function renderProfileChooser(force = false) {
  ...
  el('profileGrid').innerHTML = profiles.length ? profiles.map(profile => {
    ...
    return `
      <article class="profile-card ${previewed ? 'selected' : ''} ${current ? 'current' : ''}" data-profile-card="${esc(profile.slug)}">
        <button class="profile-card-main" type="button" data-profile-preview="${esc(profile.slug)}" aria-expanded="${previewed ? 'true' : 'false'}">
          <span class="profile-card-copy">
            ...
            <span class="profile-preview-hint">${previewed ? 'Interesses exibidos abaixo' : 'Toque para ver os interesses considerados'}</span>
          </span>
          <span class="profile-card-chevron">${previewed ? '⌃' : '⌄'}</span>
        </button>
        ${previewed ? `
          <div class="profile-preview">
            ${profile.interests ? `<div class="profile-interests"><b>Interesses de referência:</b><br>${esc(profile.interests)}</div>` : '<div class="profile-interests">Interesses ainda não cadastrados.</div>'}
            <button class="profile-confirm" type="button" data-profile-confirm="${esc(profile.slug)}">${current ? 'Continuar com este perfil' : 'Usar este perfil'}</button>
          </div>
        ` : ''}
      </article>
    `;
  ...
  el('profileGrid').querySelectorAll('[data-profile-preview]').forEach(button => {
    button.onclick = () => {
      state.profileCandidateSlug = button.dataset.profilePreview;
      renderProfileChooser(true);
    };
  });
  el('profileGrid').querySelectorAll('[data-profile-confirm]').forEach(button => {
    button.onclick = () => selectProfile(button.dataset.profileConfirm);
  });
...
function hydrateLoadedData(data) {
  ...
  if (el('profileCriteriaButton')) {
    el('profileCriteriaButton').hidden = false;
    el('profileCriteriaButton').textContent = 'Interesses considerados';
  }
