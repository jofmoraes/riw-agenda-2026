function bindList() {
  document.querySelectorAll('.event').forEach(card => {
    ...
    card.querySelector('.details-button').onclick = () => {
      if (state.expanded.has(eventId)) state.expanded.delete(eventId); else state.expanded.add(eventId);
      render();
    };
...
function openModal(eventId) {
  ...
  el('modalContent').innerHTML = `<div class="modal-meta">${esc(event.day)} · ${esc(event.start || '--:--')}–${esc(event.end || '')}${locationText ? ` · ${esc(locationText)}` : ''}</div><label class="decision-field modal-decision-field"><span>Nível de interesse</span><select class="modal-select" aria-label="Nível de interesse">${decisionOptions(decision(event))}</select></label><div class="badges">${badges(event)}</div>${details(event)}`;
  el('modalBackdrop').classList.add('open');
  bindEditor(el('modalContent'), eventId);
}

function clearAdvancedFiltersPreservingAgenda() {
  state.tag = '';
  state.priority = '';
  state.potential = '';
  state.audioRoom = '';
...
el('clearSearch').onclick = () => { state.search = ''; render(); el('search').focus(); };
...
el('spaceFilter').onchange = event => { state.space = event.target.value; state.reviewMode = false; render(); };
el('stageFilter').onchange = event => { state.stage = event.target.value; state.reviewMode = false; render(); };
...
el('toggleAllStatuses').onclick = () => { state.visibleStatuses = new Set(STATUS_OPTIONS); state.reviewMode = false; render(); };
el('clearStatuses').onclick = () => { state.visibleStatuses = new Set(); state.reviewMode = false; render(); };
el('selectInterestStatuses').onclick = () => { state.visibleStatuses = new Set(INTEREST_STATUS_OPTIONS); state.reviewMode = false; render(); };
el('clearFilters').onclick = () => { clearAdvancedFiltersPreservingAgenda(); render(); };
el('filtersToggle').onclick = () => { state.filtersOpen = !state.filtersOpen; render(); };
el('filterClose').onclick = closeAdvancedFilters;
el('filterBackdrop').onclick = closeAdvancedFilters;
...
window.addEventListener('keydown', event => {
  if (event.key === 'Escape') {
    ...
    if (state.filtersOpen) closeAdvancedFilters();
  }
});
