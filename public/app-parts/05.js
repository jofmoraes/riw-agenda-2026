...
function renderLegend() {
  const statusItems = STATUS_OPTIONS.map(status => {
    ...
    const colorMap = {'status-go':['var(--go)','var(--go-bg)'],'status-high':['var(--high)','var(--high-bg)'],'status-interest':['var(--interest)','var(--interest-bg)'],'status-maybe':['var(--maybe)','var(--maybe-bg)'],'status-review':['var(--review)','var(--review-bg)'],'status-online':['var(--online)','var(--online-bg)'],'status-no':['var(--no)','var(--no-bg)'],'status-unreviewed':['var(--unreviewed)','var(--unreviewed-bg)']};
...
function updateToolbar(events) {
  ...
  if (el('filterBackdrop')) el('filterBackdrop').classList.toggle('open', state.filtersOpen);
  document.body.classList.toggle('filters-open', state.filtersOpen && window.matchMedia('(max-width: 860px)').matches);
...
function render() {
  syncControls(); renderDayPicker(); renderStatuses(); renderLegend();
  const events = filteredEvents();
  renderSummary(events); updateToolbar(events);
  state.view === 'grid' ? renderGrid(events) : renderList(events);
  updateStickyOffset(); persistState();
}

function save(eventId, patch, statusElement, done) {
  if (statusElement) statusElement.textContent = 'Salvando…';
  apiCall('updateEvent', {profile: state.profile.slug,eventId,patch: JSON.stringify(patch || {})})
    .then(updated => {
      ...
      cacheProfileData(state.profile.slug, {profiles: state.profiles,selectedProfile: state.profile,events: state.events});
      if (statusElement) statusElement.textContent = 'Salvo';
      if (done) done(updated);
    })
    .catch(error => { if (statusElement) statusElement.textContent = `Erro: ${error.message || error}`; else alert(error.message || error); });
}
...
function bindEditor(container, eventId) {
  ...
  if (select) select.onchange = () => save(eventId, {decision: select.value, review: ''}, status, () => render());
  if (saveButton && comments) saveButton.onclick = () => save(eventId, {comments: comments.value}, status);
}
